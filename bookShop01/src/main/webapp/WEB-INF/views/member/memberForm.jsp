...
function fn_overlapped(){
 ...
 function fn_checkPwd(){
	 ...
	 $.ajax({
		 type:"post",
		 url:"${contextPath}/member/checkPwd.do",
		 dataType:"text",
		 data:{pwd:_pwd},
		 ...
		 error:function(data, textStatus, errorThrown){
			 if(data.status === 400){
				 alert("비밀번호가 12자리 미만입니다.");
			 }else if(data.status === 500){
				 alert("특수문자를 포함시켜야합니다.");
			 }
		 }
	 });
 }
...
<body>
	<h3>필수입력사항</h3>
	<form id="formObj" action="${contextPath}/member/addMember.do" method="post" onsubmit="return fn_check_data()">	
	<div id="detail_table">
		<table>
			<tbody>
				<tr class="dot_line">
				...
				<tr class="dot_line">
					...
					<td><input id="member_pw" name="member_pw" type="password" size="20" />
					<input type="button" value="비밀번호체크" onClick="fn_checkPwd()">
					</td>
						<div id="showResult">
							<div id="content"></div>
						</div>
