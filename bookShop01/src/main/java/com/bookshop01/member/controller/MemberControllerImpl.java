...
public class MemberControllerImpl extends BaseController implements MemberController{
	...
	public ModelAndView login(@RequestParam Map<String, String> loginMap,
		...
		if(memberVO!= null && memberVO.getMember_id()!=null){ //회원정보 찾았을 때
	...
	public ResponseEntity addMember(@ModelAttribute("memberVO") MemberVO _memberVO,
		...
		System.out.println("sts 값:" + _memberVO.getEmailsts_yn());
		String message = null;
		ResponseEntity resEntity = null;
		HttpHeaders responseHeaders = new HttpHeaders();
		responseHeaders.add("Content-Type", "text/html; charset=utf-8");
		try {
			boolean dub_checker =memberService.addMember(_memberVO);
			System.out.println("dub_checker 값 " + dub_checker);
		    if(dub_checker == true) {
		    	 message  = "<script>";
				 message +=" alert('중복된 회원정보가 존재합니다.');";
	...
	public ResponseEntity checkPwd(@RequestParam("pwd") String pwd, HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if(pwd.length()<12 && pwd.length()>0) {
			message="비밀번호가 12자리 미만입니다.";
			//jsonObject.put("message", message);
			resEntity= new ResponseEntity(message,responseHeaders, HttpStatus.BAD_REQUEST);
			return resEntity;
		}else {
			if(pwd.matches(".*[!@#\\$%^&*_].*") != true) {
				message="특수문자를 포함시켜야 합니다.";
